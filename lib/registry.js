...
function load(file) {
  ...
  services.forEach((service) => {
    ...
    // Optional: which repo directory under workspaceRoot holds cli/svctl, and
    // which build name `svctl jenkins run-pipeline containerize` takes. Both
    // end up in an argv array, so they go through the same flag-safety gate as
    // everything else here.
    if (service.repo !== undefined && service.repo !== null) {
      assertSafe(service.repo, 'repo', service.name);
    }

    if (service.build !== undefined && service.build !== null) {
      assertSafe(service.build, 'build', service.name);
    }
