...
/**
 * Builds the argv for a JSON merge patch of one namespaced object. Merge, not
 * strategic, so the patch replaces only the fields it names.
 *
 * @param {Object} target
 * @param {string} kind
 * @param {string} name
 * @param {string} patch an already serialised JSON merge patch
 * @returns {Array<string>}
 */
function mergePatchArgs(target, kind, name, patch) {
  const timeoutSeconds = target.timeoutSeconds || 60;
...
/**
 * Serialises a suspend patch. Kept in one place so the boolean is always a real
 * JSON boolean: kubectl rejects `"true"`, and a stray string would be worse if
 * it did not.
 *
 * @param {boolean} suspend
 * @returns {string}
 */
function suspendPatch(suspend) {
  return JSON.stringify({ spec: { suspend } });
}

/**
 * Suspends or resumes a single Job, leaving its CronJob's schedule alone.
 *
 * This backs the per-pod "stop this job" action: one running CronJob pod is
 * picked and only its Job is stopped. Suspend is deliberate and must not be
 * "simplified" into a delete -- the user is shown, and accepts, that suspending
 * deletes the Job's live pods and that resuming restarts the work from the
 * beginning. A delete would instead destroy the Job itself, with no way back.
 *
 * @param {Object} target
 * @param {string} job
 * @param {boolean} suspend
 * @returns {Promise<{ message: string } | { error: Object }>}
 */
async function setJobSuspend(target, job, suspend) {
  const timeoutSeconds = target.timeoutSeconds || 60;
  const result = await executor.run(
    KUBECTL,
    mergePatchArgs(target, 'job', job, suspendPatch(suspend)),
    { timeout: timeoutSeconds * 1000 + EXECUTOR_GRACE_MS }
  );
  ...
  return {
    message: result.stdout.trim() || `${job} ${suspend ? 'suspended' : 'resumed'}`,
  };
}

/**
 * Lists the Jobs a CronJob has spawned.
 *
 * @param {Object} target
 * @param {string} cronjob
 * @returns {Promise<{ jobs: Array<Object> } | { error: Object }>}
 */
async function getJobsForCronJob(target, cronjob) {
  const timeoutSeconds = target.timeoutSeconds || 60;
  const result = await executor.run(
    KUBECTL,
    [
  ...
  return {
    jobs: items
      .filter((item) => {
        const owner = ((item.metadata && item.metadata.ownerReferences) || [])[0];

        return Boolean(owner) && owner.kind === 'CronJob' && owner.name === cronjob;
      })
      .map((item) => ({
        name: item.metadata.name,
        suspended: (item.spec || {}).suspend === true,
        // A Job that has already completed is history; nothing about it can be
        // suspended or resumed.
        finished: Boolean((item.status || {}).completionTime),
      })),
  };
}

/**
 * Suspends or resumes a CronJob's schedule, and optionally its Jobs with it.
 *
 * Suspending the CronJob alone only stops future runs. Suspending a Job that is
 * RUNNING deletes its live pods, and resuming it creates fresh ones, so the
 * work starts over from the beginning rather than continuing where it stopped.
 * That is why the Jobs are opt-in through a separate flag instead of moving
 * with the schedule: this is not a harmless toggle.
 *
 * @param {Object} target
 * @param {string} cronjob
 * @param {boolean} suspend
 * @param {boolean} [includeActiveJobs]
 * @returns {Promise<{ steps: Array<string> } | { error: Object, steps: Array<string> }>}
 */
async function setCronJobSuspend(target, cronjob, suspend, includeActiveJobs) {
  const timeoutSeconds = target.timeoutSeconds || 60;
  const options = { timeout: timeoutSeconds * 1000 + EXECUTOR_GRACE_MS };
  const patch = suspendPatch(suspend);
  const verb = suspend ? 'suspended' : 'resumed';
  const steps = [];

  const patched = await executor.run(
    KUBECTL,
    mergePatchArgs(target, 'cronjob', cronjob, patch),
    options
  );

  if (patched.code !== 0) {
    return { error: classifyError(patched.stderr), steps };
  }

  steps.push(`${cronjob} schedule ${verb}`);

  if (!includeActiveJobs) {
    return { steps };
  }

  const listed = await getJobsForCronJob(target, cronjob);

  if (listed.error) {
    return { error: listed.error, steps };
  }

  // Patching a Job that is already in the wanted state is a no-op on the
  // cluster but still reads as an action in the step list, so skip it.
  const pending = listed.jobs.filter((job) => !job.finished && job.suspended !== suspend);

  if (pending.length === 0) {
    steps.push(`no running jobs to ${suspend ? 'suspend' : 'resume'}`);

    return { steps };
  }

  // Sequential: a failure halfway has to stop, and the steps already taken are
  // what the UI reports back.
  for (const job of pending) {
    const result = await executor.run(
      KUBECTL,
      mergePatchArgs(target, 'job', job.name, patch),
      options
    );

    if (result.code !== 0) {
      return { error: classifyError(result.stderr), steps };
    }

    steps.push(`${job.name} ${verb}`);
  }
