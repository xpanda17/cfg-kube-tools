...
/**
 * The controller that created the pod. Kubernetes allows several owners, but a
 * pod has exactly one controller, and it is the first reference in practice.
 *
 * @param {Object} pod
 * @returns {Object|null}
 */
function firstOwner(pod) {
  return ((pod.metadata && pod.metadata.ownerReferences) || [])[0] || null;
}

/**
 * Names the Job that created the pod, for pods that came from one.
 *
 * Read from the ownerReference rather than the pod name: `<job>-<suffix>` is
 * only a convention, and an indexed Job adds a segment (`<job>-<index>-<suffix>`),
 * so splitting the name is ambiguous.
 *
 * @param {Object} pod
 * @returns {string|null}
 */
function jobName(pod) {
  const owner = firstOwner(pod);

  return owner && owner.kind === 'Job' ? owner.name : null;
}
...
function groupName(pod) {
  ...
  const owner = firstOwner(pod);
  const ownerName = (owner && owner.name) || pod.metadata.name;

  // Six of the cron pods lack the parent label, so fall back to the Job name
  // with its schedule slot suffix removed.
  return parent && ownerName.startsWith(parent) ? parent : ownerName.replace(/-\d+$/, '');
}
